...
def fn_filter_netcdf_layers(str_netcdf_path, str_out_dir, flt_threshold):
    ...
    # --- Remove flow bands that share the same flow value --------------------
    # Keep one band per unique flow (first occurrence, original order preserved).
    # Duplicate-flow bands - e.g. a boundary flow written by two adjacent
    # flow-range segments - would otherwise create a zero-flow-increment step
    # that corrupts the consecutive-layer difference and depth-gradation logic
    # below.  To keep the lowest-WSEL copy of a duplicate instead of the first,
    # see the B_KEEP_LOWEST_WSEL_DUP switch note.
    arr_flow_all = ds['flow'].values
    _, arr_first_pos = np.unique(arr_flow_all, return_index=True)
    arr_keep_pos = np.sort(arr_first_pos)
    if len(arr_keep_pos) < len(arr_flow_all):
        #print("  " + str_filename + ": removed "
        #      + str(len(arr_flow_all) - len(arr_keep_pos))
        #      + " duplicate-flow band(s)")
        ds = ds.isel(flow=arr_keep_pos)
    # -------------------------------------------------------------------------
