...
# ***********************
def fn_get_bc_boundary_cells_dict(hdf_path, mesh_name=None, filter_to_mesh=True):
    """
    Parameters
    ----------
    hdf_path : str
        Path to the HEC-RAS geometry (*.gNN.hdf) or plan (*.pNN.hdf) file.
    mesh_name : str, optional
        Name of the 2D flow area to use. Defaults to the first one found.
    filter_to_mesh : bool
        If True, keep only the external faces whose BC line belongs to
        ``mesh_name`` (face indices in External Faces are local to each area,
        so this matters when a model has more than one 2D area).

    Returns
    -------
    dict with keys:
        'mesh_name'              : the 2D area used
        'external_face_indices'  : sorted list of external face indices
        'face_to_cell'           : {face_index: interior_cell_index}
        'boundary_cell_indices'  : sorted, de-duplicated list of cells that
                                   have a face on an external BC line
    """
    with h5py.File(hdf_path, "r") as hf:
        geom = hf["Geometry"]
        areas_grp = geom["2D Flow Areas"]

        # 2D-area names are the sub-GROUPS (siblings like "Attributes" are datasets)
        area_names = [k for k in areas_grp.keys()
                      if isinstance(areas_grp[k], h5py.Group)]
        if not area_names:
            raise ValueError("No 2D Flow Areas found in the file.")

        if mesh_name is None:
            mesh_name = area_names[0]
        elif mesh_name not in area_names:
            raise ValueError(
                f"'{mesh_name}' not found. Available: {area_names}")

        # (n_faces, 2): the two cells bordering each face of this mesh
        faces_cell_idx = areas_grp[mesh_name]["Faces Cell Indexes"][()]

        # --- external faces --------------------------------------------------
        ext = geom["Boundary Condition Lines"]["External Faces"][()]
        names = ext.dtype.names
        if not names or "Face Index" not in names:
            raise KeyError(
                "'External Faces' has no 'Face Index' field. "
                f"Fields present: {names}")

        face_indices = np.asarray(ext["Face Index"], dtype=np.int64)
        bc_line_ids = (np.asarray(ext["BC Line ID"], dtype=np.int64)
                       if "BC Line ID" in names else None)

        # --- optionally keep only faces that belong to this 2D area ----------
        if filter_to_mesh and bc_line_ids is not None:
            try:
                bc_attrs = geom["Boundary Condition Lines"]["Attributes"][()]
                # find the field that names the owning 2D/storage area
                area_field = next(
                    (f for f in bc_attrs.dtype.names
                     if any(t in f for t in ("SA-2D", "SA/2D", "2D Flow Area",
                                             "Storage", "Mesh"))),
                    None)
                if area_field is not None:
                    owner = bc_attrs[area_field]
                    owner = np.array([v.decode() if isinstance(v, bytes) else v
                                      for v in owner])
                    keep = owner[bc_line_ids] == mesh_name
                    face_indices = face_indices[keep]
            except Exception:
                pass  # fall back to using every external face

        # --- map each face to its interior cell ------------------------------
        face_to_cell = {}
        for fi in face_indices:
            a, b = faces_cell_idx[fi]
            cands = [int(c) for c in (a, b) if c >= 0]
            face_to_cell[int(fi)] = min(cands) if cands else None

        boundary_cells = sorted({c for c in face_to_cell.values()
                                 if c is not None})

    return {
        "mesh_name": mesh_name,
        "external_face_indices": sorted(int(f) for f in face_indices),
        "face_to_cell": face_to_cell,
        "boundary_cell_indices": boundary_cells,
    }
# ***********************
...
def fn_create_models_nextgen_gpkg(str_config_file_path,
    ...
    #gdf_2d_area_polygon.set_crs(epsg=32127, allow_override=True, inplace=True)
    ...
    # Set the minimum flow to int(flt_q_min) from the config file -- on the points layer of GPKG
    gdf_upstream_points['q_lower_limit'] = gdf_upstream_points['q_lower_limit'].apply(
        lambda x: int(flt_min_q) if x < int(flt_min_q) else x)

    # make sure that the q_lower_limit is less than the q_upper_limit
    mask = gdf_upstream_points['q_lower_limit'] >= gdf_upstream_points['q_upper_limit']
    gdf_upstream_points.loc[mask, 'q_upper_limit'] = gdf_upstream_points.loc[mask, 'q_lower_limit'] + 1

    # Create a coloumn of idx_model_cell -- For each point in gdf_upstream_points
    # this is the cell (polygon) in gdf_cell that contains the point
    # Use spatial join to find which cell each point falls into
    print('   -- Determining flow point starting cells...')
    gdf_cells = gdf_cells.to_crs(gdf_upstream_points.crs)

    gdf_upstream_points_with_cell = gpd.sjoin(
        gdf_upstream_points, 
        gdf_cells[['geometry', 'idx']],
        how='left', 
        predicate='within'
    ).rename(columns={'idx': 'idx_model_cell'})
    
    # Sjoing atomatically created an 'index_right' that we don't want
    gdf_upstream_points_with_cell = gdf_upstream_points_with_cell.drop(columns=['index_right'])
    
    # Drop any row whose idx_model_cell is a boundary cell -- as HEC-RAS will fail to run
    # if emitter is moved to a outflow boundary condition cell
    dict_boundary_cells = fn_get_bc_boundary_cells_dict(str_hdf_file_path)
    #print('-------------------')
    #print("Boundary cells:", len(dict_boundary_cells["boundary_cell_indices"]))
    #print('-------------------')
    
    boundary_indices = dict_boundary_cells["boundary_cell_indices"]
    gdf_upstream_points_with_cell = gdf_upstream_points_with_cell[
        ~gdf_upstream_points_with_cell['idx_model_cell'].isin(boundary_indices)].copy()
    ...
    # HARD CODED OUTPUT NAME OF GEOPACKAGE
    ...
    gdf_upstream_points_with_cell.to_file(str_gpkg_to_create, layer='02_flow_points', driver="GPKG")
