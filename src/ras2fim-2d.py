    ...
    for root, dirs, files in os.walk(str_model_path_02):
        for file in files:
            if file.endswith(".p01.hdf"):
                ...
                dest_file_path = os.path.join(str_hdf_path_03, file)
...
def fn_ras2fim_2d(str_source_folder, 
    ...
    str_model_path_02b = os.path.join(str_output_dir, '02b_prep_for_ras')
    ...
    if int_start_step <= 5 and int_end_step >= 5:
        ...
        # Running step 4b to determine the rasters that are valid to merge
        fn_determine_fims_per_reach(str_output_step_04,b_print_output)
    ...
    if int_start_step <= 6 and int_end_step >= 6:
        ...
        # Note:  Step 05 script is multithreaded on a 'per-run' basis
        # Therefore, this step is running in serial with each 'run' running in parallel
