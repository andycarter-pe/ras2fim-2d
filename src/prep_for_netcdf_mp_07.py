...
def fn_create_netcdf(list_align_rasters, str_type, str_id, str_input_dir, str_terrain_filepath):
    ...
    for tiff_file in sorted_tif_files:
        # The wsel rasters are stored as uint16 tenths-of-a-foot with nodata
        # 65535 (see script 06).  masked=True turns 65535 into NaN; dividing by
        # 10 returns the data to feet, so the cube is held in feet in memory and
        # packed back to uint16 tenths on write via the scale_factor encoding.
        da = rioxarray.open_rasterio(tiff_file, masked=True)
        ...
        # convert stored tenths-of-a-foot back to feet
        da = da / 10.0

        # Assign a name to the DataArray
        da.name = str_type

        # rioxarray tags rasters with scale_factor/add_offset in attrs; remove
        # them so our packing encoding can set scale_factor without conflict
    ...
    # Load the terrain raster as a DataArray (feet).  masked=True converts the
    # nodata sentinel to NaN so it packs to the 65535 fill on write.
    terrain_da = rioxarray.open_rasterio(str_terrain_filepath, masked=True)
    terrain_da = terrain_da.squeeze()
    terrain_da = terrain_da.rename("terrain")

    # remove rioxarray's scale_factor/add_offset attrs so packing can set them
    terrain_da.attrs.pop('scale_factor', None)
    terrain_da.attrs.pop('add_offset', None)
    ...
    # --- pack wsel and terrain as uint16 tenths-of-a-foot with a CF scale ----
    # The data are held in feet in memory; scale_factor=0.1 stores them as
    # uint16 tenths on disk, while GDAL/QGIS and xarray auto-apply the scale so
    # values read back - and display - in feet.  _FillValue 65535 is the nodata,
    # and grid_mapping keeps each variable linked to the CRS (spatial_ref) var.
    # NOTE: uint16 x 0.1 caps at 6553.5 ft and cannot store negatives - fine for
    # these reaches, but a datum with negative or very high elevations would clip.

    # capture the CRS before restructuring, then build a Dataset so the global
    # attributes below are written at the file level (not on a variable)
    crs_out = da_xr.rio.crs
    ds_out = da_xr.to_dataset(name=str_type)

    # Add global attributes to the file
    ds_out.attrs['00_stream_id'] = str_id
    ds_out.attrs['01_type'] = str_type
    ds_out.attrs['02_description'] = 'RAS2FIM-2D Raster Stack'
    ds_out.attrs['03_author'] = 'Created by Andy Carter, PE'
    ds_out.attrs['04_organization'] = 'Center for Water and the Environment'
    ds_out.attrs['05_institution'] = 'University of Texas at Austin'
    ds_out.attrs['06_history'] = f"Created on {datetime.now().strftime('%Y-%m-%d %H:%M')}"
    ds_out.attrs['07_raster_folder'] = str_input_dir

    # re-assert the CRS so the spatial_ref grid-mapping variable is written
    if crs_out is not None:
        ds_out = ds_out.rio.write_crs(crs_out)

    # build the packing encoding for the variables that are present
    dict_encoding = {}
    for str_var in (str_type, 'terrain'):
        if str_var in ds_out.variables:
            dict_encoding[str_var] = {'dtype': 'uint16',
                                      'scale_factor': 0.1,
                                      '_FillValue': 65535,
                                      'grid_mapping': 'spatial_ref'}

    # Save the dataset as a NetCDF file (single write - preserves CRS + scale)
    ds_out.to_netcdf(str_netcdf_filepath,
                     format='NETCDF4',
