...
def fn_generate_depth_wsel_rasters(str_config_file_path,
    ...
    # Interim... need to loop
    int_count = 0
    
    """
    # This appears to be running slower??
    with concurrent.futures.ThreadPoolExecutor(max_workers=4) as executor:
        futures = {executor.submit(fn_process_raster_stack, 
                                   str_wsel_col_name, 
                                   gdf_cells_wsel,
                                   gdf_center_points,
                                   flt_pixel_width,
                                   str_output_dir): str_wsel_col_name for str_wsel_col_name in list_wsel_col_names}
        for future in concurrent.futures.as_completed(futures):
            int_count += 1
            str_wsel_col_name = futures[future]
            try:
                result = future.result()
                print('--------')
                print(f'Run {int_count} of {len(list_wsel_col_names)} completed: {result}')
            except Exception as e:
                print(f'Run {int_count} of {len(list_wsel_col_names)} generated an exception: {str_wsel_col_name} - {e}')

    """
    
    # TODO - should this processing should be in parallel?
    for str_wsel_col_name in list_wsel_col_names:
        
        int_count += 1
        print('--------')
        print(f'Run {int_count} of {len(list_wsel_col_names)}')
        ...
        print('Computing Depth and WSEL Raster...')
        list_created_rasters = fn_raster_subtraction(list_raster_to_purge[2],
                                                     list_raster_to_purge[3],
                                                     str_output_dir,
                                                     str_wsel_col_name)
        ...
        gdf_floodplain = fn_make_floodplain_gdf(list_depth_raster_path[0], flt_minvalue, flt_max_value)
        
        # Save the gdf_floodplain as a gpkg
        str_flood_limits_gpkg = os.path.join(str_output_dir, '09_' + str_wsel_col_name + '_floodplain_ar.gpkg')
...
if __name__ == '__main__':
    ...
    parser.add_argument('-i',
                        ...
                        help=r'REQUIRED: path to plan HDF file (HDF) Example: E:\HECRAS_2D_12070205\base_model_20240414_copy\BLE_LBSG_501.p04.hdf',
                        required=False,
                        default=r'E:\HECRAS_2D_12070205\base_model_20240414_copy\BLE_LBSG_501.p04.hdf',
                        metavar='FILE',
                        type=lambda x: is_valid_file(parser, x))
    
    parser.add_argument('-t',
                        dest = "str_dem_path",
                        help=r'REQUIRED: path to terrain dem (TIF) Example: E:\HECRAS_2D_12070205\BLE_12070205_Engineering_Models\Engineering Models\Hydraulic Models\RAS_Submittal\LBSG_501\Input\Terrain\Terrain4.DEM_3.tif',
                        required=False,
                        default=r'E:\HECRAS_2D_12070205\BLE_12070205_Engineering_Models\Engineering Models\Hydraulic Models\RAS_Submittal\LBSG_501\Input\Terrain\Terrain4.DEM_3.tif',
                        metavar='FILE',
                        type=lambda x: is_valid_file(parser, x))
    
    parser.add_argument('-g',
                        dest = "str_hydraulic_results_gpkg",
                        help=r'REQUIRED: hydraulic results geopackage (geopackage) Example: E:\sample_2d_output\BLE_LBSG_501_p02\hydrualic_results_1884413_wb-2410249_wb-2410261_30-hr_500-cfs_to_3000-cfs_step_600-cfs.gpkg',
                        required=False,
                        default=r'E:\sample_2d_output\BLE_LBSG_501_p02\hydrualic_results_1884413_wb-2410249_wb-2410261_30-hr_500-cfs_to_3000-cfs_step_600-cfs.gpkg',
                        metavar='FILE',
                        type=lambda x: is_valid_file(parser, x))
    
    parser.add_argument('-o',
                        dest = "str_output_dir",
                        help=r'REQUIRED: directory to write model hydrofabric geopackage Example: E:\working\large_dem',
                        required=False,
                        default=r'E:\working\large_dem',
                        metavar='DIR',
                        type=str)
