...
def fn_travel_time_and_peak_flow_est(gdf_streams,
    ...
    for index, row in gdf_waterbody_flowpaths_disolve.iterrows():
        ...
        if len(list_linestrings) > 0:
            # Delete row in gdf_mainstems_revised where 'mainstem' = mainstem_value
    ...
    # For each mainstem (with Waterbodies removed, determine the low flow travel 
    # time estiamte for that mainstem run) - Also, get the 'id' of the downstream
    # stream name
    list_travel_time_hr = []
    list_downstream_stream_id = []
    ...
    for index, row in gdf_mainstems_revised.iterrows():
        ...
        # Determine the stream name 'id' for the most downstream
        # stream reach in this 'mainstem' run
        
        # determine the last stream (should have the biggest area and be the most downstream)
        str_downstream_stream_id = sorted_covered_rows.iloc[-1]['id']
        list_downstream_stream_id.append(str_downstream_stream_id)
    ...
    gdf_mainstems_revised['id_end_node'] = list_downstream_stream_id
