...
# Revised - 2025.04.06
# Revised to remove the 'controlled precipitation' to replace with the
# original version of the 'emitter' internal boundary condition
# ************************************************************
...
# --------------
def fn_build_log_plan_names(dict_mainstem):
    str_run_name, str_short_plan = None, None
    
    # Example:  str_run_name = '1884413_wb-2410249_wb-2410261_29-hr_14100-cfs_to_45000-cfs_logstep_45_minq_100-cfs'
    ...
    try:
        ...
        if not is_single_flow:  
            ...
            # add the logstep to the name of the run
            #str_run_name += "_logstep_" + str(int_log_q_steps)
            
            # add the logstep to the name of the run
            #str_run_name += "_minq_" + str(int(flt_min_q)) + "-cfs"

        str_short_plan = str_start_node + '-' + fn_format_flow_as_string(int(flt_lower_flow))
    except:
        pass
...
def fn_set_plan_simulation_date_range(str_copy_to_p_full_path,
    ...
    # added 2025.04.04 -- padding an hours to 'bound' precip
    int_sim_time_hr += 1
...
# -------------
def fn_generate_log_step_integer(arr_values, flt_lower_flow, flt_upper_flow):
    # Filter values between the lower and upper flow limits
    arr_filtered_values = arr_values[(arr_values >= flt_lower_flow) & (arr_values <= flt_upper_flow)]
    
    # Check if the filtered list is empty
    if arr_filtered_values.size == 0:
        # No values found between given numbers
        arr_filtered_values = np.insert(arr_filtered_values, 0, flt_lower_flow)
        arr_filtered_values = np.append(arr_filtered_values, flt_upper_flow)
    
    # Ensure flt_lower_flow is the first value and flt_upper_flow is the last value
    if arr_filtered_values[0] != flt_lower_flow:
        arr_filtered_values = np.insert(arr_filtered_values, 0, flt_lower_flow)
    if arr_filtered_values[-1] != flt_upper_flow:
        arr_filtered_values = np.append(arr_filtered_values, flt_upper_flow)
    
    list_filtered_values = arr_filtered_values.tolist()
...
def fn_copy_ras_file(tup_of_file_params,
    ...
    if not df_filtered.empty:
        ...
        if str_header == 'g':
            ...
            str_b_geom = str_copy_from[:-3] + 'b' + str_copy_from[-2:]
            ...
            str_copy_b_full_path = str_copy_to_full_path[:-3] + 'b' + str_copy_to_full_path[-2:]
            
            #shutil.copy(str_c_geom, str_copy_c_full_path)
            shutil.copy(str_x_geom, str_copy_x_full_path)
            shutil.copy(str_b_geom, str_copy_b_full_path)
...
# ----------------------
def fn_create_prj(str_run_name, str_project_path, str_new_folder, str_list_flows, str_log_parameter):
    ...
    str_description += '------------------------------' + '\n'
    str_description += str_list_flows + '\n'
    str_description += str_log_parameter + '\n'
    # ~~~~~~~~~~
...
def fn_copy_source_terrain(str_folder_for_copies, str_new_terain_folder_name, str_source_geom_hdf):
    ...
    if os.path.exists(str_source_geom_hdf):
        ...
        with h5py.File(str_source_geom_hdf, 'r') as hdf_file:
            ...
            str_terrain_filename = str_terrain_filename.replace("\\", "/")
...
def fn_create_time_list(str_start_time, int_delta_hr, list_peak_flows):
    int_len = len(list_peak_flows) + 1 # --- added 2025.04.04
...
def fn_create_cumulative_rainfall_array(dict_params):
    ...
    # ------Updated : 2025.04.04 -----
    # Prepend a row of zeros (same number of columns as existing data)
    # This is a 'zeroed' cummulative value at the first timestep
    zero_row = np.zeros((1, array_flows_reshaped.shape[1]))
    array_flows_reshaped = np.vstack([zero_row, array_flows_reshaped])
    # ------Updated : 2025.04.04 -----
...
def fn_revise_rainfall_hdf(dict_for_precip, projection_wkt):
    ...
    # list of the flows per the logarithmic progression
    arr_flow_values = fn_logarithmic_progression(dict_for_precip['min_q'],
                                                 dict_for_precip['upper_flow'],
                                                 dict_for_precip['log_q_steps'])
    
    list_peak_flows = fn_generate_log_step_integer(arr_flow_values,
                                                   dict_for_precip['lower_flow'],
                                                   dict_for_precip['upper_flow'])

    # Copy and append the last peak flow to the list
    #list_peak_flows.append(list_peak_flows[-1])
    ...
    if b_all_cells_unique:
        ...
        #list_peak_flows.append(list_peak_flows[-1])  # -- added one flow step 2025.04.04
...
def fn_build_internal_boundary_text(hdf_file_path, list_unique_indices_sorted, str_2d_area_name):
    ...
    # Create a numpy array of the the shapely line coordinates
...
# ---------------------------
def fn_create_hydrograph_lines(dict_flows, list_flows_int):
    ...
    list_flow_steps_int = list_flows_int
    ...
    # repeat the last value one more time to complete time window of emitter
    # added 2025.04.07
    list_flows.append(list_flows[-1])
...
# ----------------
def fn_edit_unsteady_flow_file(str_flow_fullpath, dict_flows, list_flows_int, str_boundary_to_edit):
    # Open the file and read its content
    with open(str_flow_fullpath, "r") as file:
    ...
    list_flow_rows, int_sim_time_hr = fn_create_hydrograph_lines(dict_flows, list_flows_int)
...
def fn_spawn_hecras_copies(str_config_file_path,
    ...
    if '02_copy_geom' in config:
        ...
        # flow parameters for logarithmic flow progression ... 2025.04.03
        flt_min_q = section.getfloat('flt_min_q', 0.0)
        flt_max_q = section.getfloat('flt_max_q', 0.0)
        int_log_q_steps = section.getint('int_log_q_steps', 0)
    ...
    with tqdm(total=len(gdf_mainstems), 
        ...
        for index, row in gdf_mainstems.iterrows():
            ...
            # create revised run names from log progression (2025.04.03)
            str_run_name,str_short_name = fn_build_log_plan_names(dict_mainstem)
            ...
            dict_flows = {
                ...
                "min_q": flt_min_q,
                "max_q": flt_max_q,
                "log_q_steps": int_log_q_steps,
                "hour_count": int_hour_count,
                "run_name": str_run_name,
                "short_name": str_short_name
            }
        
            # revised 2025.04.03 - Log progression of flows between bounding values
            arr_flow_values = fn_logarithmic_progression(flt_min_q, flt_max_q, int_log_q_steps)
            int_num_of_flows = len(fn_generate_log_step_integer(arr_flow_values, dict_flows['lower_flow'], dict_flows['upper_flow']))
            
            list_flows = fn_generate_log_step_integer(arr_flow_values, dict_flows['lower_flow'], dict_flows['upper_flow'])
            list_flows_int = [int(f) for f in list_flows]
            str_list_flows = 'Simulated Flows: ' + str(list_flows_int)
            str_log_parameter = 'Log_steps:' + str(int_log_q_steps) + ' Q_min:' + str(int(flt_min_q)) + ' Q_max:' + str(int(flt_max_q))
            
            # revised 2025.04.04 -- did not discount the last flow
            int_sim_time_hr = int_hour_count * (int_num_of_flows)
            ...
            fn_create_prj(str_run_name, str_project_path, str_new_folder, str_list_flows, str_log_parameter)
            ...
            str_flow_fullpath = fn_adjust_unsteady_flow(str_run_name, str_new_folder, dict_flows)
            
            # Adjusting the geometry
            str_geom_path = fn_adjust_geom(str_run_name, str_new_folder, dict_flows)
            
            # Adjusting the geometry.hdf
            str_geom_path_hdf = fn_adjust_geom_hdf(str_run_name, str_new_folder, str_terrain_relativepath)
            
           
            # ---- Added for internal boundary condition -----
            # ************************************************************
            
            list_unique_indices_sorted = [row['idx_model_cell']]
            ...
            list_new_boundary_lines = fn_build_internal_boundary_text(str_geom_path_hdf, list_unique_indices_sorted, str_2d_area_name)
            
            # replace boundary condition lines in geometry file
            fn_replace_boundary_lines_in_file(str_geom_path, list_new_boundary_lines, index)
            
            # need to edit the 'boundary line' and 'hydrograph' in the unsteady flow file
            fn_edit_unsteady_flow_file(str_flow_fullpath, dict_flows, list_flows_int, str_boundary_to_edit)
            ...
            epsg_code = crs.to_epsg()  # Note that this could be 'None' ... need to error trap this
            str_output_crs = f"EPSG:{epsg_code}"
            
            # TODO - 2025.02.12 -- What if the str_output_crs is 'EPSG:None'
            # This is due likely to the base HEC-RAS model being set up in
            # a non-compliant EPSG code
            ...
            '''
            # Removing the precip --- 2025.04.06 --- 
