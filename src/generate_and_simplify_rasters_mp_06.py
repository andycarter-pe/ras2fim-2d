...
# ================== PLAN-DRIVEN OUTPUT FILTER (2026.09.06) ==================
# This script now writes ONLY the rasters listed in a review plan CSV produced
# by plan_rasters_06.py.  The plan decides, per reach, which flow steps are
# valid (in the reach's own mainstem span) and resolves the boundary-flow
# overwrites between adjacent flow-range segments.  This script contains no
# span logic of its own -- it just obeys the plan, so the plan CSV is the
# single place to review or override what gets written.
#
# STR_PLAN_CSV : path to the plan.  Leave None to auto-use
#                <folder of the -i gpkg>\raster_write_plan.csv
STR_PLAN_CSV = None
#
# B_REMOVE_STALE_RASTERS : if True, delete *.tif already in 01_wsel_rasters
#   that are not anywhere in the plan (clears junk from earlier runs).  Each
#   deletion is printed.  Default False -- nothing is deleted unless you opt in.
B_REMOVE_STALE_RASTERS = False
# ===========================================================================
    ...
    if os.path.exists(str_expected_floodplain_filepath):
        ...
        if os.path.exists(str_expected_wsel_filepath):
            ...
            with rxr.open_rasterio(str_expected_wsel_filepath) as xds_raw_dem:
                ...
                # Example - a cell value of 4256 = a wsel of 425.6 units (feet or meters)
                # NOTE: uint16 caps at 65535, so the storable wsel ceiling is 6553.5
                xds_reproject_scaled = ((xds_reproject * 10) + 0.5) // 1
        ...
        if b_compute_depth_raster:
            if b_depth_raster_found:
                ...
                for index, row in gdf_flowpath_limits_3857.iterrows():
                    ...
                    # --- plan filter: only write (reach, flow) pairs the plan marked WRITE ---
                    if set_allowed_keys is not None:
                        try:
                            _key = (str(row['flowpath']), int(round(float(row['flow_cfs']))))
                        except (TypeError, ValueError):
                            _key = None
                        if _key not in set_allowed_keys:
        ...
        if b_wsel_raster_found:
            ...
            for index, row in gdf_flowpath_limits_3857.iterrows():
                ...
                try:
                    ...
                    # compress and write out wsel raster as unsigned 16 bit integer
                    xds_clipped_wsel.rio.to_raster(str_wsel_filepath,compress='lzw',dtype="uint16")
...
def fn_process_flow_step(dict_of_parameters):
    ...
    return fn_create_raster_products(dict_of_parameters['int_flow_step'],
                                     ...
                                     dict_of_parameters.get('set_allowed_keys'))
# -------------
...
# ****************************
def fn_load_plan(str_plan_csv, str_this_gpkg_basename):
    """
    Read the review plan CSV (from plan_rasters_06.py) and return, for the gpkg
    currently being processed, the set of (flowpath, int_flow) pairs marked
    WRITE.  Also returns the GLOBAL set of WRITE output filenames (all gpkgs),
    used only for optional stale-file cleanup.
    """
    df_plan = pd.read_csv(str_plan_csv)

    # normalise
    df_plan['flowpath'] = df_plan['flowpath'].astype(str)
    df_plan['flow_cfs'] = df_plan['flow_cfs'].astype(float).round().astype(int)

    df_write = df_plan[df_plan['decision'] == 'WRITE']

    # this gpkg's allowed (reach, flow) keys
    df_this = df_write[df_write['source_gpkg'].astype(str) == str_this_gpkg_basename]
    set_keys = set(zip(df_this['flowpath'], df_this['flow_cfs']))

    # global set of planned output filenames (for cleanup)
    if 'output_name' in df_write.columns:
        set_global_names = set(df_write['output_name'].astype(str))
    else:
        set_global_names = set('wsel_' + df_write['flowpath'] + '_'
                               + df_write['flow_cfs'].astype(str) + '.tif')

    return set_keys, set_global_names
# ****************************
...
def fn_extract_and_simplify_rasters(str_config_file_path,
    ...
    # --- load the review plan and get this gpkg's allowed (reach, flow) keys ---
    if STR_PLAN_CSV is not None:
        str_plan_csv = STR_PLAN_CSV
    else:
        str_plan_csv = os.path.join(os.path.dirname(str_flood_limits_gpkg),
                                    'raster_write_plan.csv')
    
    if not os.path.exists(str_plan_csv):
        print("  !! plan CSV not found: " + str_plan_csv)
        print("  !! Run plan_rasters_06.py first (or set STR_PLAN_CSV). Aborting.")
        return
    
    str_this_gpkg = os.path.basename(str_flood_limits_gpkg)
    set_allowed_keys, set_global_names = fn_load_plan(str_plan_csv, str_this_gpkg)
    
    if b_print_output:
        print("  --- plan: " + str_plan_csv)
        print("  --- this gpkg (" + str_this_gpkg + ") -> "
              + str(len(set_allowed_keys)) + " raster(s) to write")
    
    # --- optional: remove stale rasters not anywhere in the plan ---
    if B_REMOVE_STALE_RASTERS:
        str_wsel_folder = os.path.join(str_output_dir, '01_wsel_rasters')
        if os.path.isdir(str_wsel_folder):
            int_removed = 0
            for str_fn in os.listdir(str_wsel_folder):
                if str_fn.lower().endswith('.tif') and str_fn not in set_global_names:
                    try:
                        os.remove(os.path.join(str_wsel_folder, str_fn))
                        int_removed += 1
                        if b_print_output:
                            print("  removed stale raster: " + str_fn)
                    except OSError as e:
                        print("  could not remove " + str_fn + ": " + str(e))
            if b_print_output:
                print("  --- removed " + str(int_removed) + " stale raster(s)")
    ...
    list_of_dict = [{'int_flow_step': i,
                     ...
                     'set_allowed_keys': set_allowed_keys}
