...
**HEC-RAS 7.0.1 is installed with Windows Python, dependencies, and saved TCU acceptance.**
...
Use a disposable model copy and `--user root` for the qualified Windows Docker Desktop route. The job edits preprocessing
...
The published 7.0.1 image prepared both LF and CRLF versions of the full 266-hour ras2fim sample on Linux. Its prepared HDF was 4,676,101 bytes and retained all 6,548 coordinate rows and populated hydraulic tables. Matching native computation produced 267 output times with finite water surfaces throughout the mesh.
...
The installed HEC-RAS 7.0.1 application retains saved TCU state `701`. The source and runtime inventory identify the exact installed files; test receipts identify the executing runtime.

Allow Docker storage for the image layers and a private Wine-prefix copy for each concurrent preparation.
...
| External [runtime.json][build-manifest] and [prefix/][build-inputs] | Matching HEC-RAS 7.0.1, Windows Python, dependencies, registry and notices; kept outside Git. |
...
python containers/hecras-prepare/bundle_profile.py \
  ...
  --destination /path/to/external/wine-7.0.1 \
  --version 7.0.1
```

The manifest must declare `"hec_ras_version": "7.0.1"`. The prepared prefix
...
docker buildx build --load --platform linux/amd64 \
  --build-context hecras_runtime=/path/to/external/wine-7.0.1 \
