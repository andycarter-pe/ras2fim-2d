...
**HEC-RAS 6.6 is installed with Windows Python, dependencies, and saved TCU acceptance.**
...
The published 6.6 image prepared both LF and CRLF versions of the full
266-hour ras2fim sample on Linux. Its prepared HDF was 2,681,108 bytes,
...
| External [runtime.json][build-manifest] and [prefix/][build-inputs] | Matching HEC-RAS 6.6, Windows Python, dependencies, registry and notices; kept outside Git. |
...
python containers/hecras-prepare/bundle_profile.py \
  ...
  --destination /path/to/external/wine-6.6 \
  --version 6.6
```

The manifest must declare `"hec_ras_version": "6.6"`. The prepared prefix
...
docker buildx build --load --platform linux/amd64 \
  --build-context hecras_runtime=/path/to/external/wine-6.6 \
