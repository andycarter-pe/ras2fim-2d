# HEC-RAS 6.6 Wine preprocessing

**HEC-RAS 6.6 is installed, TCU acceptance is saved, and preprocessing has been tested. The bundled image is available as `v4` and `latest`.**
...
|---|---|
| Windows path seen by HEC-RAS and Python | `C:\Program Files (x86)\HEC\HEC-RAS\6.6\Ras.exe` |
| Linux path inside the image template | `/runtime/wine-seed/prefix/drive_c/Program Files (x86)/HEC/HEC-RAS/6.6/Ras.exe` |
| Linux path used by one running job | `/run/ras-job/<run-id>/wineprefix/drive_c/Program Files (x86)/HEC/HEC-RAS/6.6/Ras.exe` |
...
Use a disposable copy and `--user root` for compatibility with Desktop and
Ubuntu WSL mounts. Docker Desktop 4.90 also passed the default UID-1000 CRLF check. The job edits preprocessing
...
```bash
docker pull rascommander/hec-ras-wine-precompute_6.6:v4
docker run --rm --user root \
  ...
  rascommander/hec-ras-wine-precompute_6.6:v4 \
...
Docker Desktop 4.90 passed all four root-user cases. See the [Windows command and Linux options][host-guide] and [test record][host-tests].
...
The selected runtime completed the retained generated-plan sample in 86.72
...
| External [runtime.json][build-manifest] and [prefix/][build-inputs] | Installed HEC-RAS 6.6 and dependencies. Links show the manifest example and layout; actual runtime contents stay outside Git. |
...
`kind: wine`, `hec_ras_version: "6.6"`, selected executable/Python/wheel
...
python containers/hecras-prepare/bundle_profile.py \
  --source /controlled/hecras-6.6-wine \
  --destination /controlled/releases/hecras-6.6 \
  --version 6.6

docker buildx build --load --platform linux/amd64 \
  --build-context hecras_runtime=/controlled/releases/hecras-6.6 \
  --build-arg HEC_RAS_VERSION=6.6 \
...
bash containers/hecras-prepare/verify_bundled_runtime.sh \
  local/hecras-6.6:review 6.6 /scratch/hecras-audit-6.6-001
```

Then use the run command above with `local/hecras-6.6:review` and a
...
```text
rascommander/hec-ras-wine-precompute_6.6@sha256:b84b1cce0c73b8422c432a11167587a79a2bc2588b7a5d54904275dcf7af7e4a
```
